...
/*   Updated: 2023/11/14 19:28:34 by nhan             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*			else if (*(str + 1) == 'p')
				ft_print_ptr(list->str, va_arg(args, unsigned long));
*/			else if (list->str[1] == 'd' || list->str[1] == 'i')
				ft_print_int(list->str, va_arg(args, int));
			else if (list->str[1] == 'u')
				ft_print_uint(list->str, va_arg(args, unsigned int));
/*			else if (list->str[1] == 'x')
				ft_print_low_hexa(list->str, va_arg(args, unsigned long));
			else if (list->str[1] == 'X')
				ft_print_up_hexa(list->str, va_arg(args, unsigned long));
			else if (list->str[1] == '%')
				ft_print_percent(list->str, va_arg(args, char *));
*/		}
		list=list->next;
	}
}
